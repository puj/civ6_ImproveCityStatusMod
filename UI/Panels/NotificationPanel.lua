...
local m_DiploIM			:table = InstanceManager:new( "DiploInstance", "Root", Controls.DiploMessages);
local m_kMessages :table = {};

-- ===========================================================================
function OnDiploMessage( title:string, msg:string, fDisplayTime:number, type:number )
	...
	pInstance.TitleLabel:SetText( title );		
	pInstance.StatusLabel:SetText( msg );		
	pInstance.Anim:SetEndPauseTime( timeToDisplay );
	pInstance.Anim:RegisterEndCallback( function() OnEndAnimDiplo(kTypeEntry,pInstance) end );
	pInstance.Anim:SetToBeginning();
	pInstance.Anim:Play();

	Controls.DiploMessages:CalculateSize();
	Controls.DiploMessages:ReprocessAnchoring();
end

-- ===========================================================================
function OnEndAnimDiplo( kTypeEntry:table, pInstance:table )
...
function RegisterHandlers()
	...
	m_notificationHandlers[NotificationTypes.DIPLOMACY_SESSION].AddSound 			= "ALERT_POSITIVE";	
...
-- ===========================================================================
function SetNotificationTextAlert( notificationEntry:NotificationType , pNotification:table )
	if (pNotification ~= nil) then		-- Because the notification storage is 'ahead' of the events, the notification may be gone (we will get an event shortly)
		local messageName:string = Locale.Lookup( pNotification:GetMessage() );	
		local summary:string = Locale.Lookup(pNotification:GetSummary());

		local title = Locale.ToUpper(messageName);	--debug
		OnDiploMessage(title, summary, 10, 98 );
	end
end
...
function OnNotificationAdded( playerID:number, notificationID:number )
	if (playerID == Game.GetLocalPlayer())	then -- Was it for us?
		...
		if pNotification ~= nil then
			...
			for k, v in pairs(getmetatable(pNotification).__index) do
				kStr = kStr .. k .. ", ";
			end
			--OnDiploMessage( kStr, 100, 98 );
			kStr = pNotification:GetTypeName();
			--OnDiploMessage( kStr, 100, 98 );
